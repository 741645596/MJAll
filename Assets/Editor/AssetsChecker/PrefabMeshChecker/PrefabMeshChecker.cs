...
/// 预制网格检测逻辑
/// </summary>
public class PrefabMeshChecker : AssetsCheckerBase<PrefabMeshAssetInfo>
{
    private AssetsCheckEditorWindow.Settings _settings;

    public PrefabMeshChecker(AssetsCheckEditorWindow.Settings settings)
    {
        _settings = settings;
    }
    ...
    public override List<PrefabMeshAssetInfo> CollectAssetInfoList()
    {
        var assets_root = Path.Combine(Application.dataPath, _settings.assetsRootPath);

        var list = new List<PrefabMeshAssetInfo>();
        ...
        {
            var info = new PrefabMeshAssetInfo(); // 信息对象初始化
            ...
            if (obj.GetComponentsInChildren<MeshRenderer>().Length == 0) // 排除预制中没有网格渲染器
                continue;
    ...
    private static void _CheckCastShadow(Transform obj, PrefabMeshAssetInfo info)
    {
        foreach (var child in obj.GetComponentsInChildren<MeshRenderer>()) // 获得预制下的所有子对象包括自身对象上的组件
        {
    ...
    /// <param name="obj"> 要遍历的预制父节点 </param>
    /// <param name="info"> 改预制保存的信息 </param>
    private static void _CheckReceiveShadows(Transform obj, PrefabMeshAssetInfo info)
    {
        foreach (var child in obj.GetComponentsInChildren<MeshRenderer>()) // 获得预制下的所有子对象包括自身对象上的组件
        {
    ...
    /// <param name="obj"> 要遍历的预制父节点 </param>
    /// <param name="info"> 改预制保存的信息 </param>
    private static void _CheckLightProbes(Transform obj, PrefabMeshAssetInfo info)
    {
        foreach (var child in obj.GetComponentsInChildren<MeshRenderer>()) // 获得预制下的所有子对象包括自身对象上的组件
        {
    ...
    /// <param name="obj"> 要遍历的预制父节点 </param>
    /// <param name="info"> 改预制保存的信息 </param>
    private static void _CheckReflectionProbe(Transform obj, PrefabMeshAssetInfo info)
    {
        foreach (var child in obj.GetComponentsInChildren<MeshRenderer>()) // 获得预制下的所有子对象包括自身对象上的组件
        {
    ...
    /// <param name="obj"> 要遍历的预制父节点 </param>
    /// <param name="info"> 改预制保存的信息 </param>
    private static void _CheckDynamicOcclusion(Transform obj, PrefabMeshAssetInfo info)
    {
        foreach (var child in obj.GetComponentsInChildren<MeshRenderer>()) // 获得预制下的所有子对象包括自身对象上的组件
        {
