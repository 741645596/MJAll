...
/// 预制网格检测规则
/// </summary>
public class PrefabMeshRule : AssetRuleBase
{
    private List<string> _res; // 缓存判定的结果要输出tips的内容

    public PrefabMeshRule()
    {
    ...
    {
        if (info is PrefabMeshAssetInfo == false)
        {
            return true;
        }

        PrefabMeshAssetInfo assetInfo = info as PrefabMeshAssetInfo; // 文件的检测信息
    ...
    private List<string> _Check(PrefabMeshAssetInfo assetInfo)
    {
        _res.Clear();
    ...
    {
        PrefabMeshAssetInfo assetInfo = info as PrefabMeshAssetInfo;
        if (assetInfo == null)
        {
            return;
        }
        ...
        foreach (var child in obj.GetComponentsInChildren<MeshRenderer>()) // 获得预制下的所有子对象包括自身对象的组件
        {
